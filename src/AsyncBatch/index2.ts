...
export default class AsyncBatch<TDataType, TResponseType> {
	...
	private readonly _events = new Events<AsyncBatch<TDataType, TResponseType>, TDataType, Awaited<TResponseType>>();
	...
	private constructor(private action: (data: TDataType) => TResponseType, options: Partial<ICreateOptions>) {
		...
		this.rateLimiter = RateLimiter.new(
			this.options.rateLimit?.maxExecution ?? 0,
			this.options.rateLimit?.msTimeRange ?? 0,
		);
	...
	private emit(data: EventBasic<AsyncBatch<TDataType, TResponseType>>): void {
		this._events.emit(data.type, data);
	}
	 ...
	 */
	private async mayWaitNewDatas(
		nextData: IteratorResult<TDataType, void>,
