...
export default class RateLimiter {
	...
	public shot() {
		...
		if (this.window.length >= this.maxShot) throw new Error("RateLimiter: Too many shots, you should call wait() before adding a new shot");

		this.window.push(new Date());
	}

	/**
	 * Check if the number of shots is lower than the maxShot
	 * @returns {boolean} true if the number of shots is lower than the maxShot
	 */
	public isAllowed(): boolean {
		...
		return this.window.length < this.maxShot;
	}
